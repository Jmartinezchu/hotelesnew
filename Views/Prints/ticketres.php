...
$sql = "SELECT r.id_reservacion, p.id as idPagoAdelante, r.costoHabitacion, h.nombre_habitacion, u.nombres, u.identificacion, p.total, DATE_FORMAT(r.created_at, '%d %M %Y ') as fecha, p.descripcion, r.descuento FROM reservaciones r INNER JOIN reservaciones_payments p ON r.id_reservacion = p.reservacionid INNER JOIN habitacion h ON r.habitacion_id=h.idhabitacion INNER JOIN usuario u ON r.cliente = u.idusuario WHERE r.id_reservacion = $idreservacion order by p.id desc limit 1";
...
<body onload="imprimir()">
    ...
    <table align="center" class="productos">
        ...
        ?>
                ...
                <tr>
                    <td><?= $nombre ?></td>
                    <td>S/. <?= formatMoney($requestVenta['total']) ?></td>
                    <td> <?= $cantidad ?></td>
                    <td>S/. <?= formatMoney($requestVenta['total']) ?></td>
                </tr>
            <?php
