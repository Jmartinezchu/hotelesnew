<?php
 ...
 class Reservations extends Controllers{
     ...
     public function getRoomsReservationArturo(){
        ...
        for($i=0; $i < count($arrData); $i++){
            ...
            if($arrData[$i]['estado_habitacion'] == 'Disponible'){
                $btnSelect .= '<a style="background:transparent; width:1000px; height:100%; cursor:pointer" onclick="agregarHabitacion('.$arrData[$i]['idhabitacion'].',\''.$arrData[$i]['nombre_habitacion'].'\','.$arrData[$i]['capacidad'].')"><p style="color:transparent;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%)">AGREGAR HABITACION</p></a>';
            }else if($arrData[$i]['estado_habitacion'] == 'Ocupada'){
                $idreservacion = $this->model->verReserva($arrData[$i]['idhabitacion']);
                $btnSelect .= '<a style="background:transparent;color:#333;cursor:pointer" class="btn btn" onclick="verReserva('.$idreservacion.')">VER</a>';
            }
     ...
     public function getRoomsReservHoras(){
        ...
        for($i=0; $i < count($arrData); $i++){
            $btnSelect = '';
            // $btnSelect .= '<a style="background:transparent; width:1000px; height:100%; cursor:pointer" onclick="agregarHabitacionHoras('.$arrData[$i]['idhabitacion'].',\''.$arrData[$i]['nombre_habitacion'].'\','.$arrData[$i]['capacidad'].')"><p style="color:transparent;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%)">AGREGAR HABITACION</p></a>';
            // $arrData[$i]['options'] = '<div class="text-center">'.$btnSelect.'</div>';
     ...
     public function setReservationsAnticipado(){
        $idreservation = $_POST['idreservacion'];
        $start_date    = $_POST['ingreso'];
        $end_date    = $_POST['salida'];
        $origin_reservation = intval($_POST['origen_reserva']);
        ...
        $price  = $_POST['total_reserva'];
        $priceRoom = $_POST['total_habitacion'];
        $tipoServicio = $_POST['idTarifasjajaja'];
        $option = '';
        $hora = 12; // Hora que deseas agregar
        $minutos = 0; // Minutos que deseas agregar
        $segundos = 0; // Segundos que deseas agregar

        // Convierte la fecha inicial en un objeto DateTime
        $fecha_start = new DateTime($start_date);
        $fecha_end = new DateTime($end_date);
        // Agrega la hora, minutos y segundos
        $fecha_start->add(new DateInterval("PT{$hora}H{$minutos}M{$segundos}S"));
        $fecha_end->add(new DateInterval("PT{$hora}H{$minutos}M{$segundos}S"));

        // Convierte la fecha resultante en una cadena con el formato deseado
        $fecha_start_formateada = $fecha_start->format("Y-m-d H:i:s");
        $fecha_end_formateada = $fecha_end->format("Y-m-d H:i:s");

        // var_dump($_POST,$fecha_start_formateada,$fecha_end_formateada);
        //     exit;
        if($idreservation == 0 || $idreservation == ''){
            
            $request_reservation = $this->model->insertReservation($fecha_start_formateada,$fecha_end_formateada,$status,$origin_reservation,$identificacion,$customer,$correo,$direccion,$idroom,$price, $priceRoom, $tipoServicio);
            $option = 1;            
        }
        if($request_reservation > 0){
            if($option == 1){
     ...
     public function updateReservationsJm(){
        $idreservation = $_POST['idreservacion'];
        $start_date    = $_POST['ingreso'];
        $end_date    = $_POST['salida'];
        $origin_reservation = intval($_POST['origen_reserva']);
        ...
        $difference = strtotime($end_date_formatted) - strtotime($start_date_formatted);
        $days_between = floor($difference / (60 * 60 * 24));

        $price_day = $price/$days_between;
        // $option = '';
        // $hora = 12; // Hora que deseas agregar
        // $minutos = 0; // Minutos que deseas agregar
        // $segundos = 0; // Segundos que deseas agregar
        ...
        var_dump($_POST,$days_between,$price_day);
            exit;
        if($idreservation == 0 || $idreservation == ''){
     ...
     public function cambiarEstado( $id_reservacion  )
     {

        //  var_dump($_REQUEST['id']);
        // var_dump($id_reservacion);
        //  $idreserva = substr($id_reservacion,-4,2);
        //  var_dump($idreserva);exit;
        $status = substr($id_reservacion,-1);
        $request_status_update = $this->model->updateStatus($id_reservacion);
     ...
     public function cambiarEstadoCheckIn( $id_reservacion  )
     {
        ...
        $request_status_update = $this->model->updateStatusOcupada($id_reservacion);
        if($request_status_update > 0) {
            $arrResponse = array('status' => true, 'msg' => 'Se cambia el estado correctamente');
        } else {
            $arrResponse = array("status" => false, "msg" => 'Oh ocurrio un error');
