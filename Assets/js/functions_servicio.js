...
document.addEventListener("DOMContentLoaded", function() {
    ...
    tableProductos = $('#tableProductos').dataTable({
        ...
        "ajax": {
            "url": " "+base_url+"/Producto/getServicios",
            "dataSrc": ""
        },
        "columns": [
            {"data":"idProducto"},
...
$('#tableProductos').dataTable();
    ...
    function SearchCodeSunat(){
        let ajaxUrl = base_url+'/CodeSunat/getSelectCodeSunatServicios';
        let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        ...
        request.onreadystatechange = function(){
            if(request  .readyState == 4 && request.status == 200){
                ...
                document.querySelector('#codesunat').value = 26701;
            
            }
        }
    }
    function SearchCodeCategoria(){
        let ajaxUrl = base_url+'/CateServicio/getSelectCateServicio';
        let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
        request.open("GET",ajaxUrl,true);
        request.send();
