<?php 
 ...
 class PrintsModel extends Mysql {
    ...
    {
        $request = array();
        $sql = "SELECT v.idventa, v.clienteid, c.identificacion,c.nombres, DATE_FORMAT(v.created_at, '%d %M %Y %h:%i') as fecha, e.nombre, m.nombre as mediopago, tc.descripcion, v.usuario FROM venta v INNER JOIN usuario c ON v.clienteid = c.idusuario INNER JOIN ventas_estado e ON v.venta_estado_id = e.id_venta_estado INNER JOIN medio_pago m ON v.medio_pago_id = m.idmediopago INNER JOIN tipo_comprobante_sunat tc ON v.tipo_comprobante = tc.id_tipo_comprobante WHERE v.idventa =  $idventa";
